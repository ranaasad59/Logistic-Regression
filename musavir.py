...
features = loadtxt('dataset.txt', delimiter=',')
X = features[:, 0:2]
y =  features[:, 2]
pos = where(y == 1)
neg = where(y == 0)
scatter(X[pos, 0], X[pos, 1], marker='o', c='r')
...
plot(plot_x, plot_y)
legend(['Decision', 'Maligenant', 'Benign'])
show()

p = prediction(array(theta), i)

print ("Training Accuracy:",((y[where(p == y)].size / float(y.size)) * 100.0), "%")
